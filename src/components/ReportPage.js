...
class ReportPage extends Component {
  ...
  render() {
    ...
    const handleDeleteReport = (pt_id) => {
      ...
      fetch(`${config.API_ENDPOINT}/reports/${pt_id}`, options).then(() => {
      ...
      <div className='full-report-container'>
        ...
        <section className='folder-body'>
          <div className='report-buttons'>
            <button
              onClick={() =>
                this.props.history.push(`/edit-report/${chosenReport.pt_id}`)
              }
            >
              Edit
            </button>
            <button onClick={() => handleDeleteReport(chosenReport.pt_id)}>
              Delete
            </button>
          </div>
